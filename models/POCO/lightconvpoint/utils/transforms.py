...
class RandomNoiseNormal(object):
    ...
    def __call__(self, data):
        ...
        # noise = np.random.normal(loc=0.0, scale=self.sigma, size=data["pos"].shape)
        # print('noise',noise)
        # data["pos"] += torch.tensor(noise, device=data["pos"].device)
        if "normal" in data:
...
class Permutation(object):
    ...
    def __call__(self, data):
        # print('type(data)',type(data)) <class 'lightconvpoint.datasets.data.Data'>
        # print('self.key',self.key)

        if self.key in data.keys(): #if self.key in data.keys:
