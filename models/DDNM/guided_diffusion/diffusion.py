...
class Diffusion(object):
    ...
    def simplified_ddnm_plus(self, model, cls_fn):
        ...
        if args.deg =='colorization':
            ...
            if use_random_mask: # added by PointDreamer author
                image_size = 256
                mask_prob = 0.8
                total = image_size ** 2
                ...
                mask = torch.ones((3,image_size,image_size), device=self.device)
                mask[:, ...] = mask_b
            ...
            raise NotImplementedError(f"degradation type not supported:{args.deg}")
    ...
    ###################################################### added by PointDreamer author
    def get_model(self, ckpt_path='models/DDNM/256x256_diffusion_uncond.pt'): # /home/me/DiffPIR/model_zoo/256x256_diffusion_uncond.pt
        config_dict = vars(self.config.model)
        model = create_model(**config_dict)
        if self.config.model.use_fp16:
            model.convert_to_fp16()
    ...
    def simplified_ddnm_inpainting(self,model,masked_imgs,masks):

        args, config = self.args, self.config
        inpainted_imgs = torch.zeros_like(masked_imgs).to(masked_imgs.device)
        # print(f'Dataset has size {len(masked_imgs)}')
        ...
        avg_psnr = 0.0
        # pbar = tqdm.tqdm(masked_imgs)
        img_id=0
        # for x_orig in pbar:
        for x_orig in masked_imgs:
            x_orig = x_orig.to(self.device)
            x_orig = data_transform(self.config, x_orig)

            A = lambda z: z * masks[img_id]
            Ap = A
            # print('masks[img_id].shape', masks[img_id].shape)
    ...
    def svd_based_ddnm_plus(self, model, cls_fn):
        ...
        if deg == 'cs_walshhadamard':
            ...
            raise ValueError(f"degradation type not supported: {deg}")
        args.sigma_y = 2 * args.sigma_y #to account for scaling to [-1,1]
        sigma_y = args.sigma_y
